# GRD: Forking GSD for Research Projects
...
GSD is installed via `npx grd`. The fork touches these files:

### 1. Commands (`.claude/commands/grd/`)

Copy all `/grd:*` commands, rename to `/grd:*`. Modifications:
...
### 2. Agents (`.claude/agents/grd/`)

Copy all GSD agents. Modifications:

- **`grd-researcher.md`**: Reframe from "investigate tech stack" to "investigate sources and methods." Source researcher additionally checks URL liveness and records acquisition methods.
- **`grd-planner.md`**: Use research task template. Enforce ≤3 sources per task (context budget). Include `<src>` blocks with explicit methods.
- **`grd-executor.md`**: Vault write replaces git commit. Source Attachment Protocol is step 1 (acquire before synthesize). Cross-check References against `-sources/` folder before completing.
- **`grd-plan-checker.md`**: Check for source duplication across notes, primary vs. secondary sources, context window fit (paper length vs. context budget), and conflicts with BOOTSTRAP.md.

### 3. Templates (`.claude/grd/templates/`)
...
### 4. Workflows (`.claude/grd/workflows/`)
...
- **Day 1**: Copy commands/agents/workflows, rename to `grd`. Modify executor, task template, verification. Write Source Attachment Protocol reference. Write templates (research-note, source-log, bootstrap).
...
Then run `/grd:new-project --auto @valuesprism-research-spec.md` and build.
