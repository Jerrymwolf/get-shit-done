...
if [ -d "$HOME/.config/opencode/gsd-local-patches" ]; then
  ...
  PATCHES_DIR="/Users/jeremiahwolf/.claude/gsd-local-patches"
fi
...
Local patches are automatically saved when you run /gsd-r:update
...
```bash
# The manifest will be regenerated on next /gsd-r:update
