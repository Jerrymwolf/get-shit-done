...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/gsd-r:update` targets the runtime that invoked it.
...
for entry in $ORDERED_RUNTIME_DIRS; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/get-shit-done-r/VERSION" ] || [ -f "./$dir/get-shit-done-r/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/get-shit-done-r/VERSION"
    LOCAL_MARKER_FILE="./$dir/get-shit-done-r/workflows/update.md"
...
for entry in $ORDERED_RUNTIME_DIRS; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/get-shit-done-r/VERSION" ] || [ -f "$HOME/$dir/get-shit-done-r/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/get-shit-done-r/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/get-shit-done-r/workflows/update.md"
...
If you've modified any GSD files directly, they'll be automatically backed up to `gsd-local-patches/` and can be reapplied with `/gsd-r:reapply-patches` after the update.
```
...
[View full changelog](https://github.com/glittercowboy/get-shit-done-r/blob/main/CHANGELOG.md)
```
</step>
...
Run /gsd-r:reapply-patches to merge your modifications into the new version.
```
