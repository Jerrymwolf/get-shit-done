<purpose>
Create all phases necessary to close gaps identified by `/gsd-r:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gsd-r:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/gsd-r:audit-milestone` first.
```
...
PHASES=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" phases list)
...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gsd-r:execute-phase {N}` — if plans already exist
...
`/gsd-r:audit-milestone` — re-audit to verify gaps closed
`/gsd-r:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gsd-r:plan-phase` next
</success_criteria>
