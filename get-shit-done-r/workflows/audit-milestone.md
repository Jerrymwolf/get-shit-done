...
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" phases list
```
...
PHASE_INFO=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="gsd-r-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/gsd-r:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gsd-r:validate-phase {N}` |

Phases needing validation: run `/gsd-r:validate-phase {N}` for each flagged phase.
...
- /gsd-r:complete-milestone {version} — proceed anyway (accept tech debt)
