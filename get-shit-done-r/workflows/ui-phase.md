...
```bash
UI_ENABLED=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /gsd-r:settings.
```
Exit workflow.
...
Recommended: run /gsd-r:discuss-phase {N} first to capture design preferences.
...
Template: /Users/jeremiahwolf/.claude/get-shit-done-r/templates/UI-SPEC.md
</output>
...
Task(
  ...
  subagent_type="gsd-r-ui-researcher",
...
Task(
  ...
  subagent_type="gsd-r-ui-checker",
...
2. Edit manually — open UI-SPEC.md in editor, re-run /gsd-r:ui-phase
...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
