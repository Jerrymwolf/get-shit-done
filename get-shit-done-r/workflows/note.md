...
- **Global scope**: `/Users/jeremiahwolf/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/gsd-r:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/jeremiahwolf/.claude/notes/`)
...
2. Glob `/Users/jeremiahwolf/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/jeremiahwolf/.claude/notes/):
  4. [2026-02-08 10:00] cross-project idea about shared config

{count} active note(s). Use `/gsd-r:note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GSD-R project. Run `/gsd-r:new-project` to initialize one."
...
status: pending
priority: P2
source: "promoted from /gsd-r:note"
created: {YYYY-MM-DD}
theme: general
---
...
<edge_cases>
1. **"list" as note text**: `/gsd-r:note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `/Users/jeremiahwolf/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/gsd-r:new-project`
