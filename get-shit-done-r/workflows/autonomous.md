...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" init milestone-op)
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/gsd-r:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/gsd-r:new-milestone` first."
...
```bash
DETAIL=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```
Skill(skill="gsd-r:plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /gsd-r:audit-milestone to review, then /gsd-r:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /gsd-r:audit-milestone to review details."
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /gsd-r:autonomous --from {next_phase}
```
...
- [ ] Audit invoked via Skill(skill="gsd-r:audit-milestone")
