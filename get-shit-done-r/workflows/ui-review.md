...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /gsd-r:execute-phase {N} first."
...
Task(
  ...
  subagent_type="gsd-r-ui-auditor",
...
- `/gsd-r:verify-work {N}` — UAT testing
- `/gsd-r:plan-phase {N+1}` — plan next phase
...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
