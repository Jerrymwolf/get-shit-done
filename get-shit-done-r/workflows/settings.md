...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" config-ensure-section
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /gsd-r:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /gsd-r:ui-phase first when frontend indicators detected." },
...
- /gsd-r:plan-phase --skip-verify — skip plan check
```
</step>
