...
- Example: `/gsd-r:remove-phase 17` → phase = 17
- Example: `/gsd-r:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /gsd-r:remove-phase <phase-number>
Example: /gsd-r:remove-phase 17
```
...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" init phase-op "${target}")
...
To abandon current work, use /gsd-r:pause-work instead.
```
...
```bash
RESULT=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/gsd-r:progress` — see updated roadmap status
