<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gsd-r-phase-researcher, gsd-r-planner, and gsd-r-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
**If `planning_exists` is false:** Error — run `/gsd-r:new-project` first.
...
```bash
PHASE_INFO=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first": Display `/gsd-r:discuss-phase {X}` and exit workflow.
...
### Spawn gsd-r-phase-researcher

```bash
PHASE_DESC=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /gsd-r:discuss-phase)
...
Task(
  ...
  subagent_type="gsd-r-phase-researcher",
...
**If found:**
1. Read template: `/Users/jeremiahwolf/.claude/get-shit-done-r/templates/VALIDATION.md`
...
<downstream_consumer>
Output consumed by /gsd-r:execute-phase. Plans need:
...
## 10. Spawn gsd-r-plan-checker Agent
...
Task(
  ...
  subagent_type="gsd-r-plan-checker",
  model="{checker_model}",
...
- /gsd-r:plan-phase {X} --research — re-research first
...
- [ ] gsd-r-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] gsd-r-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gsd-r-plan-checker spawned with CONTEXT.md
