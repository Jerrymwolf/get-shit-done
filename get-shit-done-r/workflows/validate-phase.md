...
If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /gsd-r:settings."
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /gsd-r:execute-phase {N} first."
...
Task(
  ...
  subagent_type="gsd-r-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `/Users/jeremiahwolf/.claude/get-shit-done-r/templates/VALIDATION.md`
...
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
