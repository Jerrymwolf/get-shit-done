# Frameworks Research Template

Template for `.planning/research/FRAMEWORKS.md` — theoretical and conceptual frameworks for the research domain.

<template>

```markdown
# Theoretical Frameworks

**Domain:** [research domain]
**Researched:** [date]
**Confidence:** [HIGH/MEDIUM/LOW]

## Dominant Framework: [Name]

### Overview
[2-3 paragraph description of the framework]

### Core Constructs
| Construct | Definition | Measurement | Key Finding |
|-----------|------------|-------------|-------------|
| [construct] | [what it means] | [how measured] | [what research shows] |

### Key Propositions
1. [Proposition]: [evidence status — supported/mixed/challenged]
2. [Proposition]: [evidence status]

### Strengths
- [Strength with evidence]

### Limitations
- [Limitation with evidence]

### Historical Development
[When proposed, how it evolved, major revisions]

## Competing Framework 1: [Name]

### Overview
[Description]

### How It Differs from [Dominant Framework]
| Dimension | [Dominant] | [This Framework] |
|-----------|------------|------------------|
| [dimension] | [position] | [position] |

### When This Framework Is Preferred
[Conditions, contexts, or questions where this framework is stronger]

## Competing Framework 2: [Name]

[Same structure as above]

## Framework Comparison

| Criterion | [Framework A] | [Framework B] | [Framework C] |
|-----------|---------------|---------------|---------------|
| Scope | [what it covers] | [what it covers] | [what it covers] |
| Empirical support | [strength of evidence] | [strength] | [strength] |
| Explanatory power | [what it explains well] | [explains well] | [explains well] |
| Blind spots | [what it misses] | [misses] | [misses] |
| Methodological approach | [typical methods] | [methods] | [methods] |

## Relationships Between Frameworks

### Complementary
- [Framework A] + [Framework B]: [how they complement each other]

### Contradictory
- [Framework A] vs [Framework C]: [where they conflict and why]

### Integrative Attempts
- [Author (Year)]: [how they tried to integrate, success/failure]

## Sources

- [Sources for framework analysis]

---
*Frameworks research for: [domain]*
*Researched: [date]*
```

</template>

<guidelines>

**Dominant Framework:**
- Every field has one — identify it clearly
- Include constructs and how they're measured
- Note both strengths and limitations honestly

**Competing Frameworks:**
- Don't just list alternatives — explain HOW they differ
- Include when each is preferred (context matters)
- Some "competing" frameworks are actually complementary

**Framework Comparison:**
- Use consistent comparison dimensions
- Include empirical support level for each
- Note blind spots — what each framework can't explain

**Relationships:**
- Complementary frameworks can be used together
- Contradictory frameworks require choosing (or acknowledging the tension)
- Note integration attempts — these are often the most interesting recent work

</guidelines>
