---
name: gsd-r-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /gsd-r:debug orchestrator.
tools: Read, Write, Edit, Bash, Grep, Glob, WebSearch
color: orange
skills:
  - gsd-r-debugger-workflow
# hooks:
#   PostToolUse:
#     - matcher: "Write|Edit"
#       hooks:
...
- `/gsd-r:debug` command (interactive debugging)
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /gsd-r:debug to resume" if context filling up.
</step>
...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" state load)
...
```bash
node "/Users/jeremiahwolf/.claude/get-shit-done-r/bin/gsd-r-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
