...
Two-tier verification criteria for GRD research notes. Verification runs in order: Tier 1 first, Tier 2 second. Both tiers must pass for a note to reach `final` status.
...
- Any FALSE: Note is incomplete. Generate fix tasks via `/grd:quick`.
...
When verification fails, generate targeted fix tasks using `/grd:quick`:
