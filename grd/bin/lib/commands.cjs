...
function cmdCommit(cwd, message, files, raw, amend, noVerify) {
  ...
  // Commit
  const commitArgs = amend ? ['commit', '--amend', '--no-edit'] : ['commit', '-m', safeMessage];
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during /grd:scope-inquiry ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
    ...
    case 'phase-dir': {
      ...
      const phasesParent = path.join(planningDir(cwd), 'phases');
...
function cmdCommitToSubrepo(cwd, message, files, raw) {
  ...
  for (const [repo, repoFiles] of Object.entries(grouped)) {
    ...
    // Commit
    const commitResult = execGit(repoCwd, ['commit', '-m', sanitizeForPrompt(message)]);
    if (commitResult.exitCode !== 0) {
      if (commitResult.stdout.includes('nothing to commit') || commitResult.stderr.includes('nothing to commit')) {
...
function cmdTodoMatchPhase(cwd, phase, raw) {
  ...
  const phaseRoadmapInfo = getRoadmapPhaseInternal(cwd, phase);
  const phaseName = phaseRoadmapInfo ? (phaseRoadmapInfo.phase_name || '') : '';
  const phaseGoal = phaseRoadmapInfo ? (phaseRoadmapInfo.goal || '') : '';
  const phaseSection = phaseRoadmapInfo ? (phaseRoadmapInfo.section || '') : '';
