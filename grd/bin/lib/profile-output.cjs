...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /grd:new-research to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  ...
  '- `/grd:conduct-inquiry` for planned phase work',
  '',
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'grd', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/grd:profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'grd', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/grd:dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GRD from ${dataSource}. Run \`/grd:profile-user --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /grd:profile-user to unlock Developer Profile.';
