...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { grd_state_version: '1.0' };
...
function cmdStateBeginPhase(cwd, phaseNumber, phaseName, planCount, raw) {
  ...
  // Update plan counter
  if (planCount) {
    const planCountNum = parseInt(planCount, 10);
    if (!isNaN(planCountNum)) {
      // Try compound format first: "Plan: 1 of N"
      const planField = stateExtractField(content, 'Plan');
      if (planField && planField.includes('of')) {
        const newPlanValue = `1 of ${planCountNum}`;
        result = stateReplaceField(content, 'Plan', newPlanValue);
        if (result) { content = result; updated.push('Plan'); }
      } else {
        // Legacy separate fields
        result = stateReplaceField(content, 'Current Plan', '1');
        if (result) { content = result; updated.push('Current Plan'); }
        result = stateReplaceField(content, 'Total Plans in Phase', String(planCountNum));
  ...
  output({ updated: updated.length > 0, fields: updated }, raw, updated.length > 0 ? 'true' : 'false');
}

/**
 * Write a WAITING.json signal file to indicate the agent is waiting for user input.
 */
...
/**
 * Ensure STATE.md content contains Note Status and Source Gaps sections.
 * Adds them before Session Continuity if missing, supporting STATE.md files
 * that predate Phase 2.
 */
function ensureStateSections(content) {
  let modified = false;

  if (!/##\s*Note Status/i.test(content)) {
    // Insert before Session Continuity, or append
  ...
  if (!/##\s*Source Gaps/i.test(content)) {
    // Insert before Session Continuity but after Note Status
    const sessionMatch = content.match(/\n(## Session Continuity)/i);
    if (sessionMatch) {
      content = content.replace(/\n(## Session Continuity)/i, `\n${SOURCE_GAPS_SECTION}\n$1`);
    } else {
...
/**
 * Parse a markdown table section into rows of cell arrays.
 * Returns { headerRow, rows } where rows are arrays of trimmed cell values.
 */
function parseTableSection(content, sectionHeading) {
  ...
  if (tableLines.length < 2) return { headerRow: tableLines[0] || '', separator: '', rows: [], sectionMatch: match };
  ...
  return { headerRow, separator, rows: dataRows, sectionMatch: match };
}

/**
 * Rebuild a markdown table section from parsed data.
 */
function rebuildTableSection(content, sectionHeading, headerRow, separator, rows) {
  const escaped = escapeRegex(sectionHeading);
  const sectionPattern = new RegExp(
    `(##\\s*${escaped}\\s*\\n)([\\s\\S]*?)(?=\\n##[^#]|\\n##\\s|$)`, 'i'
  );

  const rowLines = rows.map(cells => `| ${cells.join(' | ')} |`);
  const newBody = `${headerRow}\n${separator}\n${rowLines.join('\n')}\n`;

  return content.replace(sectionPattern, (_match, header) => `${header}${newBody}`);
}

function cmdStateAddNote(cwd, options, raw) {
  const statePath = planningPaths(cwd).state;
  if (!fs.existsSync(statePath)) { error('STATE.md not found'); return; }

  const { note, status, sources, date } = options;
  if (!note || !status) { error('note and status required'); return; }

  if (!VALID_NOTE_STATUSES.includes(status)) {
    error(`Invalid status "${status}". Must be one of: ${VALID_NOTE_STATUSES.join(', ')}`);
    return;
  }

  let content = fs.readFileSync(statePath, 'utf-8');
  const ensured = ensureStateSections(content);
  content = ensured.content;
  if (ensured.modified) {
    writeStateMd(statePath, content, cwd);
    content = fs.readFileSync(statePath, 'utf-8');
  }
  const parsed = parseTableSection(content, 'Note Status');
  if (!parsed) { error('Note Status section not found in STATE.md'); return; }

  const dateVal = date || new Date().toISOString().split('T')[0];
  const sourcesVal = sources || '0';

  // Check if note already exists
  const existingIdx = parsed.rows.findIndex(r => r[0] === note);
  if (existingIdx >= 0) {
    // Update existing row
    parsed.rows[existingIdx] = [note, status, sourcesVal, dateVal];
  } else {
    // Add new row
    parsed.rows.push([note, status, sourcesVal, dateVal]);
  }

  content = rebuildTableSection(content, 'Note Status', parsed.headerRow, parsed.separator, parsed.rows);
  writeStateMd(statePath, content, cwd);
  output({ added: true, note, status, sources: sourcesVal, date: dateVal }, raw, 'true');
}

function cmdStateUpdateNoteStatus(cwd, note, status, raw) {
  const statePath = planningPaths(cwd).state;
  if (!fs.existsSync(statePath)) { error('STATE.md not found'); return; }
  ...
  const existingIdx = parsed.rows.findIndex(r => r[0] === note);
  if (existingIdx < 0) {
    output({ updated: false, reason: `Note "${note}" not found in Note Status table` }, raw, 'false');
    return;
  }

  parsed.rows[existingIdx][1] = status;
  parsed.rows[existingIdx][3] = new Date().toISOString().split('T')[0];

  content = rebuildTableSection(content, 'Note Status', parsed.headerRow, parsed.separator, parsed.rows);
  writeStateMd(statePath, content, cwd);
  output({ updated: true, note, status }, raw, 'true');
}

function cmdStateGetNotes(cwd, raw) {
  const statePath = planningPaths(cwd).state;
  if (!fs.existsSync(statePath)) { error('STATE.md not found'); return; }
  ...
  output({ notes }, raw, JSON.stringify(notes, null, 2));
}

// --- Source Gap Operations ---

function cmdStateAddGap(cwd, options, raw) {
  const statePath = planningPaths(cwd).state;
  if (!fs.existsSync(statePath)) { error('STATE.md not found'); return; }

  const { note, source, reason, impact } = options;
  if (!note || !source) { error('note and source required'); return; }
  ...
  const parsed = parseTableSection(content, 'Source Gaps');
  if (!parsed) { error('Source Gaps section not found in STATE.md'); return; }

  parsed.rows.push([note, source, reason || '', impact || '']);

  content = rebuildTableSection(content, 'Source Gaps', parsed.headerRow, parsed.separator, parsed.rows);
  writeStateMd(statePath, content, cwd);
  output({ added: true, note, source, reason: reason || '', impact: impact || '' }, raw, 'true');
}

function cmdStateResolveGap(cwd, note, source, raw) {
  const statePath = planningPaths(cwd).state;
  if (!fs.existsSync(statePath)) { error('STATE.md not found'); return; }
  ...
  parsed.rows = parsed.rows.filter(r => !(r[0] === note && r[1] === source));
  const removed = before - parsed.rows.length;

  content = rebuildTableSection(content, 'Source Gaps', parsed.headerRow, parsed.separator, parsed.rows);
  writeStateMd(statePath, content, cwd);
  output({ resolved: removed > 0, note, source, removed }, raw, removed > 0 ? 'true' : 'false');
}

function cmdStateGetGaps(cwd, raw) {
  const statePath = planningPaths(cwd).state;
  if (!fs.existsSync(statePath)) { error('STATE.md not found'); return; }
  ...
  output({ gaps }, raw, JSON.stringify(gaps, null, 2));
}
