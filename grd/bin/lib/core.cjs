...
function loadConfig(cwd) {
  ...
  const defaults = {
    ...
    // GRD research-specific defaults
  ...
  try {
    ...
    // Migrate legacy "multiRepo: true" boolean to sub_repos array
    ...
    return {
      ...
      // GRD research-specific config fields
...
function extractCurrentMilestone(content, cwd) {
  ...
  if (!version) {
    // Check for in-progress marker (construction sign emoji)
    const inProgressMatch = content.match(/\u{1F6A7}\s*\*\*v(\d+\.\d+)\s/u);
    if (inProgressMatch) {
  ...
  const nextMilestonePattern = new RegExp(
    `^#{1,${headingLevel}}\\s+(?:.*v\\d+\\.\\d+|\u2705|\u{1F4CB}|\u{1F6A7})`,
    'miu'
  );
