...
Valid GRD subagent types (use exact names — do not fall back to 'general-purpose'):
...
5. Commit:
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `/Users/jeremiahwolf/.claude/grd/templates/VALIDATION.md`
