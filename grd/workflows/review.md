...
Write to a temp file: `/tmp/grd-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/grd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/grd-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/grd-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/grd-review-claude-{phase}.md
```

**Codex:**
```bash
codex -p "$(cat /tmp/grd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/grd-review-codex-{phase}.md
```
...
Commit:
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
