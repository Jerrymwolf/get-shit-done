...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" init phase-op "${target}")
...
**Delegate the entire removal operation to grd-tools:**

```bash
RESULT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), grd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
- Don't manually renumber — use `grd-tools phase remove` which handles all renumbering
