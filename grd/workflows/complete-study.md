...
7. Archive presentation artifacts (`*-PRESENTATION-SPEC.md`, `*-OUTPUT-REVIEW.md`) alongside other phase documents
8. Clean up `.planning/reviews/` temporary files (never archived)
...
**Note:** MILESTONES.md entry is now created automatically by `grd-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to grd-tools:**

```bash
ARCHIVE=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/grd:cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `/Users/jeremiahwolf/.claude/grd/templates/retrospective.md`.
...
**Commit:**
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
