...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" init phase-op "${after_phase}")
...
**Delegate the phase insertion to grd-tools:**

```bash
RESULT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phase insert "${after_phase}" "${description}")
```
