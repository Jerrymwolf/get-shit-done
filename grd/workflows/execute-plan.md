...
@/Users/jeremiahwolf/.claude/grd/references/git-integration.md
</required_reading>
...
# config settings can be fetched via grd-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="grd-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phases list --type summaries --raw
...
See `/Users/jeremiahwolf/.claude/grd/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node /Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See /Users/jeremiahwolf/.claude/grd/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/grd/bin/grd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/grd/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/Users/jeremiahwolf/.claude/grd/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Users/jeremiahwolf/.claude/grd/templates/summary.md`.
...
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state update-progress

# Record execution metrics
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state record-metric \
...
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/grd:complete-study` + `/grd:verify-inquiry` + `/grd:add-phase` |
