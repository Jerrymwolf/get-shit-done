...
```bash
PHASE_DESC=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
- options:
  - "Design presentation first" → Display: "Run `/grd:presentation-design {N}` then re-run `/grd:plan-inquiry {N}`". Exit workflow.
