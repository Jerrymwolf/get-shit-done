...
Post-mortem investigation for failed or stuck GRD workflows. Analyzes git history,
...
3. {Recovery command if applicable — e.g., `/grd:resume-work`, `/grd:conduct-inquiry N`}

---

*Report generated by `/grd:forensics`. All paths redacted for portability.*
```
...
```bash
grd-tools.cjs state record-session \
