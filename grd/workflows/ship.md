...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
