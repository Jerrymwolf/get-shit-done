...
**Check for global defaults** at `~/.grd/defaults.json`. If the file exists, offer to use saved defaults:

```
AskUserQuestion([
  {
    question: "Use your saved default settings? (from ~/.grd/defaults.json)",
    header: "Defaults",
    multiSelect: false,
    options: [
...
If "Yes": read `~/.grd/defaults.json`, use those values for config.json, and skip directly to **Commit config.json** below.

If "No" or `~/.grd/defaults.json` doesn't exist: proceed with the questions below.
