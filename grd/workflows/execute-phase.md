...
- **Claude Code:** Uses `Task(subagent_type="grd-executor", ...)` — blocks until complete, returns result
...
These are the valid GRD subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- grd-executor — Executes plan tasks, commits, creates SUMMARY.md
- grd-verifier — Verifies phase completion, checks quality gates
- grd-planner — Creates detailed plans from phase scope
- grd-phase-researcher — Researches technical approaches for a phase
- grd-plan-checker — Reviews plan quality before execution
- grd-debugger — Diagnoses and fixes issues
...
Check if the current runtime is Copilot by testing for the `@grd-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/jeremiahwolf/.claude/grd/workflows/execute-plan.md` **inline**
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For grd-tools commits: add --no-verify flag.
       ...
       @/Users/jeremiahwolf/.claude/grd/references/checkpoints.md
       @/Users/jeremiahwolf/.claude/grd/references/tdd.md
       </execution_context>
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
...
```bash
POST_PLAN_INDEX=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
Commit the file:
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/grd:progress` and `/grd:audit-uat`.
...
```bash
COMPLETION=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/grd:progress` and `/grd:audit-uat`.
```

```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `/Users/jeremiahwolf/.claude/grd/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/grd:transition` command. Never suggest it. The transition workflow is internal only.**
