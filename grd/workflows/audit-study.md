<purpose>
Verify milestone achieved its definition of done by aggregating phase verifications, checking cross-phase integration, and assessing requirements coverage. Reads existing VERIFICATION.md files (phases already verified during conduct-inquiry), aggregates tech debt and deferred gaps, then spawns integration checker for cross-phase wiring.
</purpose>
...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" find-phase 01 --raw)
...
Discovery only — never auto-calls `/grd:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/grd:validate-phase {N}` |

Phases needing validation: run `/grd:validate-phase {N}` for each flagged phase.
