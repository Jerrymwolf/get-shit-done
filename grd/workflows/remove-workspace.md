<purpose>
Remove a GRD workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/grd:list-workspaces` to show available workspaces, then ask:
