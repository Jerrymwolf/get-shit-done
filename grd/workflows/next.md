<purpose>
Detect current project state and automatically advance to the next logical GRD workflow step.
...
```bash
# Get state snapshot
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
▶ **Next step:** `/grd:[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/grd:next` is zero-friction advancement.
</step>
