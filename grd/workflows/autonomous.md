...
**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/grd:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/grd:new-milestone` first."
...
```bash
PHASE_STATE=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
