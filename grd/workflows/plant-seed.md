...
Seeds auto-surface during /grd:new-milestone when trigger conditions match the
...
This seed should be presented during `/grd:new-milestone` when the milestone
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /grd:new-milestone
