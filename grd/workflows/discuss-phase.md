...
Use /grd:progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
```bash
TODO_MATCHES=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" resolve-model grd-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @$HOME/.claude/agents/grd-advisor-researcher.md for your role and instructions.
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/Users/jeremiahwolf/.claude/grd/bin/grd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
